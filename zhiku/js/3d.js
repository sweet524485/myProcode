...
//轮播图函数
function MoveImg1(time) {
	//定时器
	var timer = null;
	//自动判断屏幕宽度
	MoveImgWidthAuto1();
	//判断的滚动的高度
	var h = $('.fl-move-content1 .move li a img').height();
	//初始化化计数器
	var i = 0;
	//获取轮播图的个数
	var totalNum = $('.fl-move-content1 .move li').length;
	//遍历出小点点
	for (var j = 0; j < totalNum; j++) {
        $('.fl-move-content1 .doc').append('<li></li>');
    }
	//第一个小点点选中
	$('.fl-move-content1 .move li').first().addClass('active');
	$('.fl-move-content1 .doc li').first().addClass('cur');
	function autoRunLeft() {
        ...
        if (i == -1) {
            i = totalNum - 1;
            $('.fl-move-content1 .move li').eq(0).addClass('currentDown').css({'transform':'rotateX(90deg)','webkitTransform':'rotateX(90deg)','MozTransform':'rotateX(90deg)','msTransform':'rotateX(90deg)','OTransform':'rotateX(90deg)'}).siblings('li').removeClass('currentDown').hide();
            $('.fl-move-content1 .move li').eq(i).show().addClass('newDown').css({'transform':'rotateX(0deg)','webkitTransform':'rotateX(0deg)','MozTransform':'rotateX(0deg)','msTransform':'rotateX(0deg)','OTransform':'rotateX(0deg)'}).siblings('li').removeClass('newDown');
        }else{
        	$('.fl-move-content1 .move li').eq(i+1).addClass('currentDown').css({'transform':'rotateX(90deg)','webkitTransform':'rotateX(90deg)','MozTransform':'rotateX(90deg)','msTransform':'rotateX(90deg)','OTransform':'rotateX(90deg)'}).siblings('li').removeClass('currentDown').hide();
            $('.fl-move-content1 .move li').eq(i).show().addClass('newDown').css({'transform':'rotateX(0deg)','webkitTransform':'rotateX(0deg)','MozTransform':'rotateX(0deg)','msTransform':'rotateX(0deg)','OTransform':'rotateX(0deg)'}).siblings('li').removeClass('newDown');
        }
        $('.fl-move-content1 .doc li').eq(i).addClass('cur').siblings('li').removeClass('cur');
    }
	function autoRunRight(){
        ...
        $('.fl-move-content1 .move li').eq(i-1).addClass('currentDown').css({'transform':'rotateX(90deg)','webkitTransform':'rotateX(90deg)','MozTransform':'rotateX(90deg)','msTransform':'rotateX(90deg)','OTransform':'rotateX(90deg)'}).siblings('li').removeClass('currentDown').hide();
        $('.fl-move-content1 .move li').eq(i).show().addClass('newDown').css({'transform':'rotateX(0deg)','webkitTransform':'rotateX(0deg)','MozTransform':'rotateX(0deg)','msTransform':'rotateX(0deg)','OTransform':'rotateX(90deg)'}).siblings('li').removeClass('newDown');
        $('.fl-move-content1 .doc li').eq(i).addClass('cur').siblings('li').removeClass('cur');
	}
	timer = setInterval(autoRunRight, time);
    //左侧点击效果
	$('.fl-move-content1 .left').click(function(){
		autoRunLeft();
	});
	//右侧点击效果
	$('.fl-move-content1 .right').click(function(){
		autoRunRight();
	});
	//点的鼠标移出效果
	$(".fl-move-content1 .doc li").each(function () {
        var that = $(this);
        that.hoverDelay({
            hoverEvent: function () {
                var indexNum = that.index();
                if(indexNum != i){
		        	$('.fl-move-content1 .move li').eq(i).addClass('currentDown').css('transform','rotateX(90deg)').siblings('li').removeClass('currentDown').hide();
        			$('.fl-move-content1 .move li').eq(indexNum).show().addClass('newDown').css('transform','rotateX(0deg)').siblings('li').removeClass('newDown');
		        }
		        i = indexNum;
    ...
    $('.fl-move-content1').hover(function () {
        clearInterval(timer);
    }, function () {
        timer = setInterval(autoRunRight, time);
    });
...
//自动判断屏幕宽度
function MoveImgWidthAuto1(){
	w = $(window).width();
	if(w <= 1500){
		var leftNum = (1500 - w)/2;
		//轮播图的高度
		$('.fl-move-content1').height(500);
		$('.fl-move-content1 .move li a>img').css({'width':'1500px','height':'500px'});
		$('.fl-move-content1 .move li a img').css('left','-'+leftNum+'px');
		$('.fl-move-content1 .left').css('left','5%');
		$('.fl-move-content1 .left').css('right','5%');
	}else{
		//轮播图的高度
		$('.fl-move-content1').height(w/3);
		$('.fl-move-content1 .move li a>img').css({'width':'1500px','height':w/3+'px'});
		$('.fl-move-content1 .left').css('left','135px');
		$('.fl-move-content1 .left').css('right','135px');
		$('.fl-move-content1 .move li a img').css('width',w+'px');
	}
...
$(function(){
	...
	if(isIE){
	    ...
	    if(safariVersion <= DEFAULT_VERSION ){
	    	$('.fl-move-content').show();
	    	$('.fl-move-content1').hide();
	        MoveImg(3000);
	    }else{
	        $('.fl-move-content1').show();
	    	$('.fl-move-content').hide();
	        MoveImg1(3000);
	    }
	}else{
	    $('.fl-move-content1').show();
